...
@ti.kernel
def FBM_advection_vis(ngrid: ti.template(), obj: ti.template(), nobj: ti.template(), config: ti.template()):
    phase_num = ti.static(config.phase_rest_density.n)
    for i in range(obj.part_num[None]):
...
def FBM_advection_gravity(obj: ti.template(), config: ti.template()):
    ...
    for i in range(obj.part_num[None]):
        for k in ti.static(range(phase_num)):
            obj.phase_acc[i, k] += config.gravity[None]
...
def FBM_convect(ngrid: ti.template(), obj: ti.template(), nobj: ti.template(), config: ti.template()):
    ...
    for i in range(obj.part_num[None]):
        ...
        for k in ti.static(range(phase_num)):
            obj.phase_acc[i, k] += obj.acce_adv[i] * ((1 - config.fbm_convection_term[None]) + (
                config.fbm_convection_term[None]*(obj.rest_density[i]/config.phase_rest_density[None][k])))
        # obj.fbm_zeta[i] = 0
        # for j in ti.static(range(phase_num)):
