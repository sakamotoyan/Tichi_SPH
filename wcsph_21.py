...
@ti.kernel
def update_vel_mid(obj: ti.template(), config: ti.template()):
    phase_num = ti.static(config.phase_rest_density.n)
    dim = ti.static(config.gravity.n)
    for i in range(obj.part_num[None]):
        for j in ti.static(range(dim)):
...
@ti.kernel
def update_vel_drag(obj: ti.template(), config: ti.template()):
    phase_num = ti.static(config.phase_rest_density.n)
    dim = ti.static(config.gravity.n)
    for i in range(obj.part_num[None]):
        for j in ti.static(range(dim)):
...
@ti.kernel
def predict_phase_transport(ngrid: ti.template(), obj: ti.template(), nobj: ti.template(), config: ti.template()):
    phase_num = ti.static(config.phase_rest_density.n)
    for i in range(obj.part_num[None]):
...
@ti.kernel
def phase_transport_check_neg(obj: ti.template()):
...
@ti.kernel
def update_lamb(obj: ti.template(), config: ti.template()):
    phase_num = ti.static(config.phase_rest_density.n)
    for i in range(obj.part_num[None]):
        if obj.flag[i]:
            mul = 1.0
...
@ti.kernel
def normalize_volume_frac(obj: ti.template(), config: ti.template()):
    phase_num = ti.static(config.phase_rest_density.n)
    for i in range(obj.part_num[None]):
        s = 0.0
        for k in ti.static(range(phase_num)):
...
def sph_step(ngrid, fluid, bound, config):
    ...
    update_vel_mid(fluid, config)
    update_vel_drag(fluid, config)
    SPH_update_pos(fluid, config)
    for l in range(2):
        predict_phase_transport(ngrid, fluid, fluid, config)
        phase_transport_check_neg(fluid)
        if fluid.general_flag[None] == 0:
            break
        update_lamb(fluid, config)
    normalize_volume_frac(fluid, config)
