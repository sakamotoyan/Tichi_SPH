...
class Particle:
    def __init__(
        ...
        self.seq_log = ti.field(ti.i32, self.part_num[None])
            ...
            if (isinstance(struct, list)):
                ...
                for struct_i in struct:
                    self.__dict__[name].append(
                        struct_i.field(shape=(self.part_num[None],)))
            else:
            ...
            if name in self.array_list:
                if (isinstance(self.__dict__[name], list)):
                    ...
                    for array in self.__dict__[name]:
                        print(sub_element_template.format(
                            f"[{iter}]", f"dtype={array.dtype}", f"{[array]}"))
                        iter += 1
                else:
                    print(element_template.format(
                        name, f"dtype={self.__dict__[name].dtype}", f"{[self.__dict__[name]]}"))
                print(dash_template.format())
        print(" ")
