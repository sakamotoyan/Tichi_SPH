...
@ti.kernel
def copy_value_to_attr(num: ti.i32, input_attr: ti.template(), output_attr: ti.template()):
    for i in range(num):
...
@ti.kernel
def cfl_condition_dt(num: ti.i32, vel: ti.template(), discretization_size: ti.f32,
                     cfl_factor: ti.f32, max_dt: ti.f32, min_divisor: ti.f32) -> ti.f32:
    output_dt = max_dt
    for i in range(num):
        vel_norm = vel[i].norm()
