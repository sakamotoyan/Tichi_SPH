        ...
        if (obj == 'fluid') or (obj == 'bound'):
            for param in scenario_buffer[obj]['objs']:
                if param['type'] == 'cube':
                    ...
                    eval(obj).push_part_seq(len(verts), int(param['color'], 16), verts, ti.Vector(param['volume_frac']), ti.Vector(param['vel']),
                                             config)
    # except Exception:
...
def sph_step(fluid, bound, config):
    ...
    # SPH_prepare_alpha(fluid, config)
    # SPH_prepare_alpha(bound, config)
    # """ IPPE SPH divergence """
    # div_iter_count = 0
    # SPH_vel_2_vel_adv(fluid, config)
    # while div_iter_count < config.iter_threshold_min[None] or fluid.compression[None] > config.divergence_threshold[None]:
    #     IPPE_adv_psi_init(fluid, config)
    ...
    #     IPPE_psi_adv_non_negative(fluid, config)
    ...
    #         break
    # SPH_vel_adv_2_vel(fluid, config)
    # """ SPH advection """
